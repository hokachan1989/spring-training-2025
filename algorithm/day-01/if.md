...
* 範囲を指定するには、`条件式1 and 条件式2` のように条件を組み合わせることができます

- 例えば、`13 <= age <= 19` のような書き方も Python では可能です

* すべての条件を網羅していることを確認しましょう
...
* 論理演算子 `not` を使うと、条件の真偽を反転させます

- `not is_weekend` は `is_weekend` が `False` の場合に `True` になります
...
* `input()` 関数は文字列を返すため、整数に変換するには `int()` 関数を使います

- 条件の順序を考慮して、最も制限の厳しい条件から評価することが一般的です
...
* ,演算子の値を比較するためには、文字列比較 ( `==` ) を使います

- 除算 ( `/` ) の場合は、ゼロによる除算を避けるための条件チェックを追加しましょう

* 各演算子に対応する計算結果を表示しましょう

- 無効な演算子が入力された場合も処理しましょう
