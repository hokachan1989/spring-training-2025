...
{% embed url="https://hokachan1989.github.io/spring-training-2025/bubble-sort.html" %}
...
* リストの最初の要素 (インデックス 0) と 2番目の要素 (インデックス 1) を比較します

- `if numbers[0] > numbers[1]:` で比較し、条件が真なら交換します

* 要素の交換は、 `numbers[0], numbers[1] = numbers[1], numbers[0]` で行います

- この処理はまだ最初の2つの要素だけに対するものです (リスト全体には適用しません)
...
* ループを使って、リストの最初から最後まで隣接する要素同士を比較します

- ただし、リストの最後の要素には隣接する次の要素がないので、ループの範囲に注意が必要です

* `for i in range(n-1):` を使うと、0 から n-2 までのインデックスを取得できます (最後の要素のインデックスは n-1 )

- 各ステップでリストの状態を表示すると、バブルソートがどのように動作するかを視覚的に理解できます
...
* 外側のループでパス回数を制御します：`for i in range(n):`

- 内側のループでは、まだソートされていない部分だけを処理します：`for j in range(n-i-1):`

* 各パス後、リストの末尾の `i` 個の要素は既に正しい位置にあります

- 処理範囲を減らすことで、不要な比較を避けることができます
...
* 各パスの開始時に `swapped = False` のフラグを設定します

- 要素を交換するときに `swapped = True` に設定します

* パス後に `if not swapped: break` で、交換が行われなかった場合はループを終了します

- パス回数をカウントして表示することで、早期終了の効果を確認できます
...
* 関数内で、これまでのステップで実装したバブルソートのロジックを組み合わせます

- 最適化 (不要な比較のスキップと早期リターンの検出) も含めると、より効率的なバブルソートになります

* 関数化することで、異なるリストに対しても再利用できるようになります
...
* 降順にソートするには、比較の条件を変更するだけです

- 昇順ソートでは `if arr[j] > arr[j+1]:` で比較していますが、

* 降順ソートでは `if arr[j] < arr[j+1]:` とします
