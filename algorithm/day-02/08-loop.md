...
```python
# range(開始値, 終了値)：開始値から終了値-1までの数値を生成
for i in range(2, 6):
    print(i)
```
...
* `range(開始値, 終了値)` は、開始値から終了値 -1 までの数字のシーケンスを生成します
...
* ネストしたループでは、外側のループが1回実行されるごとに、内側のループが完全に実行されます
...
#### 問題1-2: range関数のバリエーション

```python
# 1. 5から10までの数字を順番に表示する
for i in range(5, 11):
    print(i)

# 2. 10から1までの数字を逆順に表示する
...
# 1. fruitsの各要素を順番に表示する
for fruit in fruits:
    print(fruit)

# 2. fruitsの各要素と、その長さ（文字数）を一緒に表示する
for fruit in fruits:
    print(f"{fruit}: {len(fruit)}文字")
```

#### 問題3-1: 条件付きループ

```python
# 1. 1から始めて10以下の数字を順番に表示するwhileループ
count = 1
while count <= 10:
    print(count)
    count += 1

# 2. 10から始めてカウントダウンし、0になったら「発射！」と表示するwhileループ
countdown = 10
while countdown >= 0:
    if countdown == 0:
        print("発射！")
